...
> boomers for the rest of the map.
- fig newtons
...
> to transition the players to the next map, the no-boomer plugin stays
> loaded in.
- 3yebex
...
{
	name = "No Boomer During Tank",
	author = "Don, epilimic, Griffin",
	description = "Prevents the director from giving the infected team a Boomer while the tank is alive",
	version = "2.0",
...
public OnPluginStart()
{
	...
	HookConVarChange(g_hBoomerLimit, Cvar_BoomerLimit);

	g_iBoomerLimit = GetConVarInt(g_hBoomerLimit);
}

public Cvar_BoomerLimit(Handle:convar, const String:oldValue[], const String:newValue[])
{
	if (g_bIsTankInPlay || StringToInt(oldValue) == 0) return;
	g_iBoomerLimit = StringToInt(newValue);
}
...
public OnMapStart()
{
	...
	if (g_iBoomerLimit > 0 && GetConVarInt(g_hBoomerLimit) == 0)
	{
		SetConVarInt(g_hBoomerLimit, g_iBoomerLimit);
	}
}
