...
#include <l4d2_hybrid_scoremod_sae>
...
// Score & Scoremod
//int iFirstHalfScore;
bool bScoremod, bHybridScoremod, bNextScoremod, bSAEScoremod;
int iMaxDistance;
...
void FindScoreMod()
{
	...
	bSAEScoremod = LibraryExists("l4d2_hybrid_scoremod_sae");
...
void FillHeaderInfo(Panel hSpecHud)
{
	...
	static char buf[64];
	// 服务器及人数显示
	Format(buf, sizeof(buf), "比赛服务器: %s [Slots %i/%i | %iT]", sHostname, GetRealClientCount(), iMaxPlayers, iTickrate);
	...
	for (int i = 0; i < total; ++i)
	{
		...
		else
		{
			if (IsHangingFromLedge(client))
			{
				// Nick: <300HP@Hanging>
				// 以下显示生还者挂边状态
				FormatEx(info, sizeof(info), "%s: <%iHP@挂边状态>", name, GetClientHealth(client));
			}
			else if (IsIncapacitated(client))
			{
				...
				// 以下显示生还者倒地状态
				Format(info, sizeof(info), "%s: <%iHP@%s> [%s %i]", name, GetClientHealth(client), (GetSurvivorIncapCount(client) == 1 ? "黑白" : "1倒"), info, GetWeaponClipAmmo(activeWep));
			}
			else
			{
				...
				else
				{
					...
					// 以下控制倒地后生还者起身必须处于流血状态的显示
					// Nick: 99HP (#1st) [Chrome 8/72]
					Format(info, sizeof(info), "%s: %iHP (#%s) [%s]", name, health, (incapCount == 2 ? "黑白" : "1倒"), info);
				}
			}
		}
	...
	switch (g_Gamemode)
	{
		...
		case GAMEMODE_SCAVENGE:
		{
			...
			if (bSecondHalf)
			{
				...
				FormatEx(info, sizeof(info), "> 对手回合用时 [%02d:%05.2f]", iMinutes, fDuration - 60 * iMinutes);
				DrawPanelText(hSpecHud, info);
			}
		}
		
		// 以下为ZoneMod专用血分系统显示HUD，可自行更改文字
		case GAMEMODE_VERSUS:
		{
			if (bHybridScoremod)
			{
			...
			else if (bScoremod)
			{
				...
				FormatEx(info, sizeof(info), "> 血量分数: %i", healthBonus);
				DrawPanelText(hSpecHud, info);
			...
			// 此血分插件不知道为哪个插件服务，有可能是ApexMod或者NeoMod
			...
			// 以下为GoldMod专用血分系统显示HUD，无需的话可以删除
			else if (bSAEScoremod)
			{
				int healthBonus	= GMPlus_GetHealthBonus(),	maxHealthBonus	= GMPlus_GetMaxHealthBonus();
				int damageBonus	= GMPlus_GetDamageBonus(),	maxDamageBonus	= GMPlus_GetMaxDamageBonus();
				int propBonus	= GMPlus_GetPropBonus(),	maxPropBonus	= GMPlus_GetMaxPropBonus();
				int throBonus	= GMPlus_GetThroBonus(),	maxThroBonus	= GMPlus_GetMaxThroBonus();
				
				int totalBonus		= healthBonus		+ damageBonus		+ propBonus		+ throBonus;
				int maxTotalBonus	= maxHealthBonus	+ maxDamageBonus	+ maxPropBonus	+ maxThroBonus;
				
				DrawPanelText(hSpecHud, " ");
				
				// PB = PropBonus / TB = ThrowablesBonu //
				// > HB: 100% | DB: 100% | PB: 60 / 100% | TB: 20 / 100%s
				// > Bonus: 860 <100.0%>
				// > Distance: 400
				
				FormatEx(	info,
							sizeof(info),
							"> HB: %.0f%% | DB: %.0f%% | PB: %i / %.0f%% | TB: %i / %.0f%%",
							L4D2Util_IntToPercentFloat(healthBonus, maxHealthBonus),
							L4D2Util_IntToPercentFloat(damageBonus, maxDamageBonus),
							propBonus, L4D2Util_IntToPercentFloat(propBonus, maxPropBonus),
							throBonus, L4D2Util_IntToPercentFloat(throBonus, maxThroBonus));
				DrawPanelText(hSpecHud, info);
				
				FormatEx(info, sizeof(info), "> [Gold] 奖励分: %i <%.1f%%>", totalBonus, L4D2Util_IntToPercentFloat(totalBonus, maxTotalBonus));
				DrawPanelText(hSpecHud, info);
				
				FormatEx(info, sizeof(info), "> [Gold] 路程分: %i", iMaxDistance);
				//if (InSecondHalfOfRound())
				//{
				//	Format(info, sizeof(info), "%s | R#1: %i <%.1f%%>", info, iFirstHalfScore, L4D2Util_IntToPercentFloat(iFirstHalfScore, L4D_GetVersusMaxCompletionScore() + maxTotalBonus));
				//}
...
void FillInfectedInfo(Panel hSpecHud)
{
	...
	switch (g_Gamemode)
	{
		case GAMEMODE_SCAVENGE:
		{
			...
			FormatEx(info, sizeof(info), "->2. 特殊感染者 [%d of %d]", score, GetScavengeRoundLimit());
		}
		case GAMEMODE_VERSUS:
		{
			FormatEx(info, sizeof(info), "->2. 特殊感染者 [%d]",
	...
	for (int client = 1; client <= MaxClients; ++client)
	{
		...
		if (!IsPlayerAlive(client)) 
		{
			...
			else // Ghost Countdown
			{
				...
				// verygood: Dead (15s)
				FormatEx(info, sizeof(info), "%s: 死亡 (%s)", name, (timeLeft ? buffer : "复活倒计时..."));
		...
		else
		{
			...
			else
			{
				...
				if (GetEntityFlags(client) & FL_ONFIRE)
				{
					...
					// 控制特感是否处于燃烧状态的HUD
					FormatEx(info, sizeof(info), "%s: %s (%iHP) [燃烧]%s", name, zClassName, iHP, buffer);
				}
				else
				{
	...
	if (!infectedCount)
	{
		DrawPanelText(hSpecHud, "特殊感染者方没有玩家.");
	}
}
...
bool FillTankInfo(Panel hSpecHud, bool bTankHUD = false)
{
	...
	if (!IsFakeClient(tank))
	{
		FormatEx(info, sizeof(info), "网络延迟: %ims / %.1f", RoundToNearest(GetClientAvgLatency(tank, NetFlow_Both) * 1000.0), LM_GetLerpTime(tank) * 1000.0);
	}
	else
	{
		info = "网络延迟: AI";
	...
	if (GetEntityFlags(tank) & FL_ONFIRE)
	{
		...
		FormatEx(info, sizeof(info), "燃烧 : %is", timeleft);
		DrawPanelText(hSpecHud, info);
	}
